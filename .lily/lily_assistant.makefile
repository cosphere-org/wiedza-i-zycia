...
#
.PHONY: lint
lint:  ## lint the wiedza_i_zycie & tests
	...
	flake8 --ignore D100,D101,D102,D103,D104,D105,D106,D107,D202,D204,W504,W606 wiedza_i_zycie
...
lily_assistant_test:
	...
	source env.sh && \
	py.test --cov=wiedza_i_zycie --cov-fail-under=${TEST_COVERAGE_THRESHOLD} -r w -s -vv $(tests)
...
lily_assistant_test_all_no_coverage_threshold:
	printf "\n>> [CHECKER] check if all tests are passing\n" && \
	source env.sh && \
	py.test --cov=wiedza_i_zycie -r w -s -vv tests && \
    coverage html -d coverage_html
